...
function save() {
  if (canSend == true) {
    ...
    const sendM = document.getElementById("send-button").innerHTML;
    document.getElementById(
      "send-button"
    ).innerHTML = `Sent <i class="uil uil-check button__icon"></i>`;
    setTimeout(() => {
      document.getElementById("send-button").innerHTML = `${sendM}`;
      canSend = true;
    }, 2500);

    updatingFile(name, email, title, text);
...
const config = {
  ...
  databaseURL: "iamafullstackdeveloper-2386f.firebaseio.com",
...
function updatingFile(name, email, title, text) {
